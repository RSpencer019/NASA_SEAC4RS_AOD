...
# Description: Generates collocation compiled_TEST.csv file for eMAS and Aeronet
#               observations
#
...
"""
Generates: Compiled_TEST.csv, Aeronet_Sites_TEST.txt
"""
...
if os.path.exists(cwd+'/Aeronet_Sites_TEST.txt'):
    os.remove(cwd+'/Aeronet_Sites_TEST.txt')


# INPUT: specify collocation criteria!
sample_radius = 3 # km
time_delta = pd.Timedelta('120 min')
...
def generate_aeronet_meta(item2): 
    ...
    for i in csvreader:
        ...
        if n == 3:
            break
        n += 1
    sites = open(cwd+'/Aeronet_Sites_TEST.txt','a')
    if os.path.getsize(cwd+'/Aeronet_Sites_TEST.txt') == 0:
...
def file_search(ext, current_location):      # lev20 or hdf input
    ...
    if ext == 'hdf':
        ...
        subdir = '/TEST'
    currentfolderdict = {'CWD':cwd+subdir}
...
def collocation(FILE_NAME):
    ...
    aeronet_sites = pd.read_csv(cwd+'/Aeronet_Sites_TEST.txt', index_col=0)
    ...
    for i in aeronet_sites.index:      
        ...
        (aeronet_sites['Longitude(decimal_degrees)'][i] <= eMAS_lon_max )
            ...
            if dist_min <= dist_max:
                ...
                print eMAS_data[cval_ind_lon][cval_ind_lat] , '      ' , FILE_NAME , "    " , i , dist_min
...
# Generation of compiled_TEST.csv file
...
labels = 'timestamp,location,longitude,latitude,elevation,longitude(index),latitude(index),HDFfile,lev20file,cval_eMAS,nval_eMAS,meanval_eMAS_550,nval_aeronet,meanval_aeronet_550_intrp,meanval_aeronet_550_intrp_r2'
for i in meanval_aeronet[0].index:  
    labels = labels + ',meanval_aeronet_' + i[4:]

if os.path.exists(cwd+'/Compiled_TEST.csv'):
    os.remove(cwd+'/Compiled_TEST.csv')
with open(cwd+'/Compiled_TEST.csv', "w") as output:  
    output.write(labels+'\n')  
